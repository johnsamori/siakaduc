 ...
 * Page class
 */
class FakultasEdit extends Fakultas
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "FakultasEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "fakultasedit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("fakultaslist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "fakultaslist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "fakultaslist") {
                            ...
                            $returnUrl = "fakultaslist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'nama_fakultas' before field var 'x_nama_fakultas'
        $val = $this->getFormValue("nama_fakultas", null) ?? $this->getFormValue("x_nama_fakultas", null);
        if (!$this->nama_fakultas->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->nama_fakultas->Visible = false; // Disable update for API request
            } else {
                $this->nama_fakultas->setFormValue($val);
            }
        }
    }
    ...
    public function restoreFormValues(): void
    {
        ...
        $this->nama_fakultas->CurrentValue = $this->nama_fakultas->FormValue;
    }
    ...
    public function renderRow(): void
    {
        ...
        // nama_fakultas
        $this->nama_fakultas->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->nama_fakultas->EditValue = !$this->nama_fakultas->Raw ? HtmlDecode($this->nama_fakultas->CurrentValue) : $this->nama_fakultas->CurrentValue;
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->nama_fakultas->Visible && $this->nama_fakultas->Required) {
                if (!$this->nama_fakultas->IsDetailKey && IsEmpty($this->nama_fakultas->FormValue)) {
                    $this->nama_fakultas->addErrorMessage(str_replace("%s", $this->nama_fakultas->caption(), $this->nama_fakultas->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // nama_fakultas
        $this->nama_fakultas->setDbValueDef($newRow, $this->nama_fakultas->CurrentValue, $this->nama_fakultas->ReadOnly);
        return $newRow;
    }
    ...
    {
        if (isset($row['nama_fakultas'])) { // nama_fakultas
            $this->nama_fakultas->CurrentValue = $row['nama_fakultas'];
        }
    }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("fakultaslist"), "", $this->TableVar, true);
        $pageId = "edit";
