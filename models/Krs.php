 ...
 * Table class for krs
 */
class Krs extends DbTable implements LookupTableInterface
{
    ...
    public DbField $tanggal_pengisian;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "krs"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // tanggal_pengisian
        $this->tanggal_pengisian = new DbField(
            $this, // Table
            'x_tanggal_pengisian', // Variable name
            'tanggal_pengisian', // Name
            '`tanggal_pengisian`', // Expression
            CastDateFieldForLike("`tanggal_pengisian`", 0, "DB"), // Basic search expression
            133, // Type
            10, // Size
            0, // Date/Time format
            false, // Is upload field
            '`tanggal_pengisian`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->tanggal_pengisian->InputTextType = "text";
        $this->tanggal_pengisian->Raw = true;
        $this->tanggal_pengisian->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $this->language->phrase("IncorrectDate"));
        $this->tanggal_pengisian->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['tanggal_pengisian'] = &$this->tanggal_pengisian;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "krs";
    }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->tanggal_pengisian->DbValue = $row['tanggal_pengisian'];
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("krslist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "krsview" => $this->language->phrase("View"),
            "krsedit" => $this->language->phrase("Edit"),
            "krsadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "KrsView",
            Config("API_ADD_ACTION") => "KrsAdd",
            Config("API_EDIT_ACTION") => "KrsEdit",
            Config("API_DELETE_ACTION") => "KrsDelete",
            Config("API_LIST_ACTION") => "KrsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("krsview", $parm);
        } else {
            $url = $this->keyUrl("krsview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("krslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("krslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("krsdelete", $parm);
        }
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->tanggal_pengisian);
                } else {
                    $doc->exportCaption($this->id);
                    $doc->exportCaption($this->mahasiswa_id);
                    $doc->exportCaption($this->tahun_akademik_id);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->tanggal_pengisian);
                    } else {
                        $doc->exportField($this->id);
                        $doc->exportField($this->mahasiswa_id);
                        $doc->exportField($this->tahun_akademik_id);
