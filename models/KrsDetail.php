...
/**
 * Table class for krs_detail
 */
class KrsDetail extends DbTable implements LookupTableInterface
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "krs_detail"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // krs_id
        $this->krs_id = new DbField(
            $this, // Table
            'x_krs_id', // Variable name
            'krs_id', // Name
            '`krs_id`', // Expression
            '`krs_id`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`krs_id`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->krs_id->InputTextType = "text";
        $this->krs_id->Raw = true;
        $this->krs_id->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->krs_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['krs_id'] = &$this->krs_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "krs_detail";
    }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->krs_id->DbValue = $row['krs_id'];
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("krsdetaillist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "krsdetailview" => $this->language->phrase("View"),
            "krsdetailedit" => $this->language->phrase("Edit"),
            "krsdetailadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "KrsDetailView",
            Config("API_ADD_ACTION") => "KrsDetailAdd",
            Config("API_EDIT_ACTION") => "KrsDetailEdit",
            Config("API_DELETE_ACTION") => "KrsDetailDelete",
            Config("API_LIST_ACTION") => "KrsDetailList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("krsdetailview", $parm);
        } else {
            $url = $this->keyUrl("krsdetailview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("krsdetailedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("krsdetaillist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("krsdetailadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("krsdetaillist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("krsdetaildelete", $parm);
        }
    }
