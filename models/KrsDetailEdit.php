 ...
 * Page class
 */
class KrsDetailEdit extends KrsDetail
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "KrsDetailEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "krsdetailedit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("krsdetaillist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "krsdetaillist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "krsdetaillist") {
                            ...
                            $returnUrl = "krsdetaillist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'krs_id' before field var 'x_krs_id'
        $val = $this->getFormValue("krs_id", null) ?? $this->getFormValue("x_krs_id", null);
        if (!$this->krs_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->krs_id->Visible = false; // Disable update for API request
            } else {
                $this->krs_id->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues(): void
    {
        ...
        $this->krs_id->CurrentValue = $this->krs_id->FormValue;
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // krs_id
            $this->krs_id->setupEditAttributes();
            $this->krs_id->EditValue = $this->krs_id->CurrentValue;
            $this->krs_id->PlaceHolder = RemoveHtml($this->krs_id->caption());
            if (strval($this->krs_id->EditValue) != "" && is_numeric($this->krs_id->EditValue)) {
                $this->krs_id->EditValue = FormatNumber($this->krs_id->EditValue, null);
            }
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->krs_id->Visible && $this->krs_id->Required) {
                if (!$this->krs_id->IsDetailKey && IsEmpty($this->krs_id->FormValue)) {
                    $this->krs_id->addErrorMessage(str_replace("%s", $this->krs_id->caption(), $this->krs_id->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->krs_id->FormValue)) {
                $this->krs_id->addErrorMessage($this->krs_id->getErrorMessage(false));
            }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // krs_id
        $this->krs_id->setDbValueDef($newRow, $this->krs_id->CurrentValue, $this->krs_id->ReadOnly);

        // matakuliah_id
        $this->matakuliah_id->setDbValueDef($newRow, $this->matakuliah_id->CurrentValue, $this->matakuliah_id->ReadOnly);
        return $newRow;
    }
    ...
    {
        if (isset($row['krs_id'])) { // krs_id
            $this->krs_id->CurrentValue = $row['krs_id'];
        }
        if (isset($row['matakuliah_id'])) { // matakuliah_id
            $this->matakuliah_id->CurrentValue = $row['matakuliah_id'];
        }
    }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("krsdetaillist"), "", $this->TableVar, true);
        $pageId = "edit";
