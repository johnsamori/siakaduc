...
class KrsEdit extends Krs
{
    ...
    // Page object name
    public string $PageObjName = "KrsEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "krsedit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("krslist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "krslist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "krslist") {
                            ...
                            $returnUrl = "krslist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'tanggal_pengisian' before field var 'x_tanggal_pengisian'
        $val = $this->getFormValue("tanggal_pengisian", null) ?? $this->getFormValue("x_tanggal_pengisian", null);
        if (!$this->tanggal_pengisian->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->tanggal_pengisian->Visible = false; // Disable update for API request
            } else {
                $this->tanggal_pengisian->setFormValue($val, true, $validate);
            }
            $this->tanggal_pengisian->CurrentValue = UnformatDateTime($this->tanggal_pengisian->CurrentValue, $this->tanggal_pengisian->formatPattern());
        }
    }
    ...
    public function restoreFormValues(): void
    {
        ...
        $this->tanggal_pengisian->CurrentValue = $this->tanggal_pengisian->FormValue;
        $this->tanggal_pengisian->CurrentValue = UnformatDateTime($this->tanggal_pengisian->CurrentValue, $this->tanggal_pengisian->formatPattern());
    }
    ...
    public function renderRow(): void
    {
        ...
        // tanggal_pengisian
        $this->tanggal_pengisian->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // tanggal_pengisian
            $this->tanggal_pengisian->setupEditAttributes();
            $this->tanggal_pengisian->EditValue = FormatDateTime($this->tanggal_pengisian->CurrentValue, $this->tanggal_pengisian->formatPattern());
            $this->tanggal_pengisian->PlaceHolder = RemoveHtml($this->tanggal_pengisian->caption());
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->tanggal_pengisian->Visible && $this->tanggal_pengisian->Required) {
                if (!$this->tanggal_pengisian->IsDetailKey && IsEmpty($this->tanggal_pengisian->FormValue)) {
                    $this->tanggal_pengisian->addErrorMessage(str_replace("%s", $this->tanggal_pengisian->caption(), $this->tanggal_pengisian->RequiredErrorMessage));
                }
            }
            if (!CheckDate($this->tanggal_pengisian->FormValue, $this->tanggal_pengisian->formatPattern())) {
                $this->tanggal_pengisian->addErrorMessage($this->tanggal_pengisian->getErrorMessage(false));
            }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // mahasiswa_id
        $this->mahasiswa_id->setDbValueDef($newRow, $this->mahasiswa_id->CurrentValue, $this->mahasiswa_id->ReadOnly);

        // tahun_akademik_id
        $this->tahun_akademik_id->setDbValueDef($newRow, $this->tahun_akademik_id->CurrentValue, $this->tahun_akademik_id->ReadOnly);

        // tanggal_pengisian
        $this->tanggal_pengisian->setDbValueDef($newRow, UnFormatDateTime($this->tanggal_pengisian->CurrentValue, $this->tanggal_pengisian->formatPattern()), $this->tanggal_pengisian->ReadOnly);
        return $newRow;
    }
    ...
    {
        if (isset($row['mahasiswa_id'])) { // mahasiswa_id
            $this->mahasiswa_id->CurrentValue = $row['mahasiswa_id'];
        }
        if (isset($row['tahun_akademik_id'])) { // tahun_akademik_id
            $this->tahun_akademik_id->CurrentValue = $row['tahun_akademik_id'];
        }
        if (isset($row['tanggal_pengisian'])) { // tanggal_pengisian
            $this->tanggal_pengisian->CurrentValue = $row['tanggal_pengisian'];
        }
    }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("krslist"), "", $this->TableVar, true);
        $pageId = "edit";
