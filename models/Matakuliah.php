...
class Matakuliah extends DbTable implements LookupTableInterface
{
    ...
    public DbField $nama_mk;
    public DbField $sks;
    public DbField $prodi_id;
    public DbField $dosen_id;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "matakuliah"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->kode_mk->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['kode_mk'] = &$this->kode_mk;

        // nama_mk
        $this->nama_mk = new DbField(
            $this, // Table
            'x_nama_mk', // Variable name
            'nama_mk', // Name
            '`nama_mk`', // Expression
            '`nama_mk`', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`nama_mk`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->nama_mk->InputTextType = "text";
        $this->nama_mk->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['nama_mk'] = &$this->nama_mk;
        ...
        $this->sks->Raw = true;
        $this->sks->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->sks->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['sks'] = &$this->sks;

        // prodi_id
        $this->prodi_id = new DbField(
            $this, // Table
            'x_prodi_id', // Variable name
            'prodi_id', // Name
            '`prodi_id`', // Expression
            '`prodi_id`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`prodi_id`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->prodi_id->InputTextType = "text";
        $this->prodi_id->Raw = true;
        $this->prodi_id->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->prodi_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['prodi_id'] = &$this->prodi_id;

        // dosen_id
        $this->dosen_id = new DbField(
            $this, // Table
            'x_dosen_id', // Variable name
            'dosen_id', // Name
            '`dosen_id`', // Expression
            '`dosen_id`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`dosen_id`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->dosen_id->InputTextType = "text";
        $this->dosen_id->Raw = true;
        $this->dosen_id->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->dosen_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['dosen_id'] = &$this->dosen_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "matakuliah";
    }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->nama_mk->DbValue = $row['nama_mk'];
        $this->sks->DbValue = $row['sks'];
        $this->prodi_id->DbValue = $row['prodi_id'];
        $this->dosen_id->DbValue = $row['dosen_id'];
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->nama_mk);
                    $doc->exportCaption($this->sks);
                    $doc->exportCaption($this->prodi_id);
                    $doc->exportCaption($this->dosen_id);
                } else {
                    $doc->exportCaption($this->id);
                    $doc->exportCaption($this->kode_mk);
                    $doc->exportCaption($this->nama_mk);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->nama_mk);
                        $doc->exportField($this->sks);
                        $doc->exportField($this->prodi_id);
                        $doc->exportField($this->dosen_id);
                    } else {
                        $doc->exportField($this->id);
                        $doc->exportField($this->kode_mk);
                        $doc->exportField($this->nama_mk);
