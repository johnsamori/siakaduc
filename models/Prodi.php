...
/**
 * Table class for prodi
 */
class Prodi extends DbTable implements LookupTableInterface
{
    ...
    // Fields
    public DbField $id;
    public DbField $nama_prodi;
    public DbField $fakultas_id;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "prodi"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // nama_prodi
        $this->nama_prodi = new DbField(
            $this, // Table
            'x_nama_prodi', // Variable name
            'nama_prodi', // Name
            '`nama_prodi`', // Expression
            '`nama_prodi`', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`nama_prodi`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->nama_prodi->InputTextType = "text";
        $this->nama_prodi->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['nama_prodi'] = &$this->nama_prodi;

        // fakultas_id
        $this->fakultas_id = new DbField(
            $this, // Table
            'x_fakultas_id', // Variable name
            'fakultas_id', // Name
            '`fakultas_id`', // Expression
            '`fakultas_id`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`fakultas_id`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->fakultas_id->InputTextType = "text";
        $this->fakultas_id->Raw = true;
        $this->fakultas_id->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->fakultas_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['fakultas_id'] = &$this->fakultas_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "prodi";
    }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->nama_prodi->DbValue = $row['nama_prodi'];
        $this->fakultas_id->DbValue = $row['fakultas_id'];
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("prodilist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "prodiview" => $this->language->phrase("View"),
            "prodiedit" => $this->language->phrase("Edit"),
            "prodiadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ProdiView",
            Config("API_ADD_ACTION") => "ProdiAdd",
            Config("API_EDIT_ACTION") => "ProdiEdit",
            Config("API_DELETE_ACTION") => "ProdiDelete",
            Config("API_LIST_ACTION") => "ProdiList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("prodiview", $parm);
        } else {
            $url = $this->keyUrl("prodiview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("prodiedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("prodilist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("prodilist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("prodidelete", $parm);
        }
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->nama_prodi);
                    $doc->exportCaption($this->fakultas_id);
                } else {
                    $doc->exportCaption($this->id);
                    $doc->exportCaption($this->nama_prodi);
                    $doc->exportCaption($this->fakultas_id);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->nama_prodi);
                        $doc->exportField($this->fakultas_id);
                    } else {
                        $doc->exportField($this->id);
                        $doc->exportField($this->nama_prodi);
                        $doc->exportField($this->fakultas_id);
