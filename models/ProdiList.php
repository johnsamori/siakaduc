...
class ProdiList extends Prodi
{
    ...
    // Page object name
    public string $PageObjName = "ProdiList";
    ...
    public string $FormName = "fprodilist";

    // CSS class/style
    public string $CurrentPageName = "prodilist";
    ...
    public function setVisibility(): void
    {
        ...
        $this->nama_prodi->setVisibility();
        $this->fakultas_id->setVisibility();
    }
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Table object (prodi)
        if (!isset($GLOBALS["prodi"]) || $GLOBALS["prodi"]::class == PROJECT_NAMESPACE . "prodi") {
            $GLOBALS["prodi"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "prodiadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "prodidelete";
        $this->MultiUpdateUrl = "prodiupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'prodi');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "prodiview"); // If View page, no primary button
                } else { // List page
...
// Is modal
		...
		if (ReadCookie('prodi_searchpanel') == 'notactive' || ReadCookie('prodi_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('prodi_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fprodisrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->nama_prodi->AdvancedSearch->toJson(), ","); // Field nama_prodi
        $filterList = Concat($filterList, $this->fakultas_id->AdvancedSearch->toJson(), ","); // Field fakultas_id
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fprodisrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field nama_prodi
        $this->nama_prodi->AdvancedSearch->SearchValue = $filter["x_nama_prodi"] ?? "";
        $this->nama_prodi->AdvancedSearch->SearchOperator = $filter["z_nama_prodi"] ?? "";
        $this->nama_prodi->AdvancedSearch->SearchCondition = $filter["v_nama_prodi"] ?? "";
        $this->nama_prodi->AdvancedSearch->SearchValue2 = $filter["y_nama_prodi"] ?? "";
        $this->nama_prodi->AdvancedSearch->SearchOperator2 = $filter["w_nama_prodi"] ?? "";
        $this->nama_prodi->AdvancedSearch->save();

        // Field fakultas_id
        $this->fakultas_id->AdvancedSearch->SearchValue = $filter["x_fakultas_id"] ?? "";
        $this->fakultas_id->AdvancedSearch->SearchOperator = $filter["z_fakultas_id"] ?? "";
        $this->fakultas_id->AdvancedSearch->SearchCondition = $filter["v_fakultas_id"] ?? "";
        $this->fakultas_id->AdvancedSearch->SearchValue2 = $filter["y_fakultas_id"] ?? "";
        $this->fakultas_id->AdvancedSearch->SearchOperator2 = $filter["w_fakultas_id"] ?? "";
        $this->fakultas_id->AdvancedSearch->save();
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->nama_prodi); // nama_prodi
            $this->updateSort($this->fakultas_id); // fakultas_id
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->id->setSort("");
                $this->nama_prodi->setSort("");
                $this->fakultas_id->setSort("");
            }
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"prodi\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"prodi\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"prodi\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fprodilist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fprodilist\" data-key=\"" . $this->keyToJson(true) .
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"prodi\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "nama_prodi");
            $this->createColumnOption($option, "fakultas_id");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fprodisrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fprodisrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fprodilist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_prodi", "data-rowtype" => RowType::ADD]);
...
// Load basic search values
    ...
    public function loadRowValues(array|bool|null $row = null): void
    {
        ...
        $this->nama_prodi->setDbValue($row['nama_prodi']);
        $this->fakultas_id->setDbValue($row['fakultas_id']);
    }
    ...
    protected function newRow(): array
    {
        ...
        $row['nama_prodi'] = $this->nama_prodi->DefaultValue;
        $row['fakultas_id'] = $this->fakultas_id->DefaultValue;
        return $row;
    }
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // nama_prodi
            $this->nama_prodi->ViewValue = $this->nama_prodi->CurrentValue;

            // fakultas_id
            $this->fakultas_id->ViewValue = $this->fakultas_id->CurrentValue;
            $this->fakultas_id->ViewValue = FormatNumber($this->fakultas_id->ViewValue, $this->fakultas_id->formatPattern());
            ...
            // nama_prodi
            $this->nama_prodi->HrefValue = "";
            $this->nama_prodi->TooltipValue = "";

            // fakultas_id
            $this->fakultas_id->HrefValue = "";
            $this->fakultas_id->TooltipValue = "";
        }
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('prodi_searchpanel') == 'notactive' || ReadCookie('prodi_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fprodisrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('prodi_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fprodisrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fprodisrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
