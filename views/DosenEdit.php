...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_dosen_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="dosen" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
loadjs.ready(['fdosenedit', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fdosenedit", "x_prodi_id", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
